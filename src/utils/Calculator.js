export const calculate = (kategori, kisiSayisi) => {
  ...
  if (kisiSayisi <= 0 || kategori === 0 || !adetSayilari[kategori]) {
    return {
      success: false,
      message: "Lütfen geçerli bir hatim ve kişi sayısı giriniz!",
    };
  }
  ...
  if (kategori === 1 && kisiSayisi > 33) {
    return {
      success: true,
      message: `33'den fazla kişi olduğu için herkes 3 adet okuyacak.`,
  ...
  if (kategori !== 1 && kisiSayisi > adet) {
    return {
      success: false,
      message:
        "Kişi sayısı, okunacak miktardan fazla. Lütfen hatmi bölerek hesaplayınız!",
    };
  }
